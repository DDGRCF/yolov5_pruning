...
class Tracker:
    ...
    def update(self, detections):
        ...
        matches, unmatched_tracks, unmatched_detections = \
            self._match(detections) # TODO: 9/27

        # Update track set.
        for track_idx, detection_idx in matches:
            # track更新对应的detection
            self.tracks[track_idx].update(self.kf, detections[detection_idx])
            self.tracks[track_idx].get_track_cls_id(detections[detection_idx].cls_id)
        ...
        # 获取所有confirmed状态的track id 这里相当于汇总
        features, targets = [], []
        for track in self.tracks:
    ...
    def _match(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            cost_matrix = self.metric.distance(features, targets) # cost_matrix.shape(len(targets, len(features))) len(features) = 1

            # 2. 计算马氏距离,得到新的状态矩阵
            cost_matrix = linear_assignment.gate_cost_matrix(
                self.kf, cost_matrix, tracks, dets, track_indices,
                detection_indices) # 这里使用马氏距离计算cost_matrix
            return cost_matrix
        ...
        iou_track_candidates = unconfirmed_tracks + [
            ...
            if self.tracks[k].time_since_update == 1  # 刚刚没有匹配上，只经过一次而没匹配上的
        ]
        # 未匹配
        unmatched_tracks_a = [
            k for k in unmatched_tracks_a
    ...
    def _initiate_track(self, detection):
        # 在第一帧进行初始化 一般是在第一帧进行的,对所有的目标的检测框而言
        mean, covariance = self.kf.initiate(detection.to_xyah()) # mean.shape(4) covariance.shape(8, 8)
        self.tracks.append(
            Track(mean, covariance, self._next_id, self.n_init, self.max_age,
                  detection.feature))
        self._next_id += 1 # 每个对象的id值经过此后+1
